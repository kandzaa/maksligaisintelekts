...
<section class="d-flex align-items-center page-hero  inner-page-hero " id="page-hero">
  ...
  <div class="container">
    <div class="hero-text-area centerd">
      <h1 class="hero-title  wow fadeInUp" data-wow-delay=".2s">{{ $news->title_en }}</h1>
      <nav aria-label="breadcrumb ">
        <ul class="breadcrumb wow fadeInUp" data-wow-delay=".6s">
          <li class="breadcrumb-item"><a class="breadcrumb-link" href="{{ route('home.en') }}"><i class="bi bi-house icon "></i>home</a></li>
          <li class="breadcrumb-item"><a class="breadcrumb-link" href="{{ route('blog.en') }}">blogs</a></li>
          <li class="breadcrumb-item active">{{ $news->title_en }}</li>
        </ul>
      </nav>
    </div>
  </div>
        ...
        <div class="blog-single-content">
          @if($news->image)
            <div class="blog-image mb-4">
              <img src="{{ asset('assets/images/news/' . $news->image) }}" alt="{{ $news->title_en }}" class="img-fluid rounded" style="max-height: 400px; width: 100%; object-fit: cover;">
            </div>
          @endif
          
          <div class="blog-meta mb-4" style="color: rgba(255, 255, 255, 0.6); font-size: 0.9rem;">
            <i class="fas fa-calendar me-2"></i>{{ $news->created_at->format('M d, Y') }}
            @if($news->updated_at->ne($news->created_at))
              <span class="ms-3"><i class="fas fa-edit me-1"></i> Updated: {{ $news->updated_at->format('M d, Y') }}</span>
            @endif
          </div>
            ...
            <div class="row align-items-center">
              <div class="col-md-6">
                <div class="author-info" style="color: rgba(255, 255, 255, 0.6);">
                  ...
                  <span>Māksligais Intelekts Team</span>
                </div>
              </div>
        ...
        <div class="navigation-links mt-5">
          <div class="row">
            ...
            <div class="col-6 text-end">
              <a href="{{ route('blog.en') }}" class="btn btn-outline-secondary" style="border-color: rgba(255, 255, 255, 0.2); color: rgba(255, 255, 255, 0.8);">
  ...
  <div class="container">
    <div class="sec-heading centered mb-5">
      <div class="content-area">
        <h2 class="title" style="color: #ffffff;">Related Articles</h2>
      </div>
    </div>
    ...
    <div class="row">
          ...
          <div class="related-post-card" style="background: #1a1a1a; border: 1px solid #2a2a2a; border-radius: 8px; padding: 1.5rem; height: 100%;">
            @if($related->image)
              <div class="related-post-image mb-3">
                <img src="{{ asset('assets/images/news/' . $related->image) }}" alt="{{ $related->title_en }}" class="img-fluid rounded" style="width: 100%; height: 150px; object-fit: cover;">
              </div>
            @endif
            <h5 style="color: #ffffff; margin-bottom: 1rem;">
              <a href="{{ route('news.show.en', $related->slug) }}" style="color: #ffffff; text-decoration: none;">
                {{ Str::limit($related->title_en, 60) }}
              </a>
            </h5>
      ...
      @empty
        <div class="col-12 text-center">
          <p style="color: rgba(255, 255, 255, 0.6);">No related articles found.</p>
        </div>
      @endforelse
    </div>
  </div>
